...
def simulate_typing(text, min_delay=0.01, max_delay=0.03):
    for char in text:
...
command_outputs = [
    ...
    ("kubectl get pods -n data-processing", [
        ...
        "data-pipeline-{hash}      1/1     Running   0          {time}",
        "spark-worker-pool-{hash}   1/1     Running   0          {time}",
        "jupyter-notebook-{hash}   1/1     Running   0          {time}",
        "airflow-scheduler-{hash}   1/1     Running   0          {time}"
    ]),
    ("airflow dags list", [
